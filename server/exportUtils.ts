...
export async function generateCSVExport(gradings: Grading[]): Promise<string> {
  const csvData = gradings.map(grading => {
    ...
    return {
      ...
      rubric_type: rubric?.name || `Rubric ${grading.rubricId}`,
      overall_score: calculateOverallScore(scores),
      essay_length_chars: grading.essayText.length,
      essay_word_count: Math.ceil(grading.essayText.length / 5),
      feedback_summary: grading.feedback.substring(0, 150) + '...',
      recommendations_count: recommendations.length,
      graded_by: 'CorestoneGrader AI',
      platform: 'CorestoneGrader',
      ...scores // Spread individual criterion scores
    };
  });

  // Add header with CorestoneGrader branding
  const headers = Object.keys(csvData[0] || {});
  const csvContent = [
    `# CorestoneGrader Essay Grading Results Export`,
    `# Generated on: ${new Date().toISOString()}`,
    `# Platform: CorestoneGrader - AI-Powered Essay Assessment`,
    `# Website: www.corestronegrader.com`,
    `# Total Essays: ${gradings.length}`,
    ``,
    headers.join(','),
    ...csvData.map(row => 
      headers.map(header => {
...
export function generateJSONExport(gradings: Grading[]) {
  ...
  return {
    platform: {
      ...
      description: "AI-Powered Essay Assessment Platform",
      website: "www.corestronegrader.com",
      technology: "OpenAI GPT-4o with Authentic IB Assessment Criteria"
    },
    export_info: {
      generated_at: new Date().toISOString(),
      total_gradings: gradings.length,
      format_version: '2.0',
      export_type: gradings.length === 1 ? 'individual' : 'bulk',
      data_source: 'CorestoneGrader Database'
    },
    gradings: gradings.map(grading => {
      ...
      return {
        ...
        assessment: {
          ...
          graded_by: "CorestoneGrader AI",
          assessment_method: "OpenAI GPT-4o with Authentic IB Criteria"
        }
      };
    })
  };
  ...
  const htmlContent = `
    ...
    <body>
      <div class="header">
        ...
        <p>AI-Powered Essay Assessment Platform</p>
        <p>Professional Essay Grading Results Report</p>
        <p>Generated for: ${username}</p>
        <p>Date: ${new Date().toLocaleDateString()}</p>
        <p>Total Essays: ${gradings.length}</p>
        <p>Assessment Technology: OpenAI GPT-4o with Authentic IB Criteria</p>
      </div>
      ...
      <div class="footer">
        <p><strong>Generated by CorestoneGrader</strong> | www.corestronegrader.com</p>
        <p>AI-Powered Essay Assessment Platform using OpenAI GPT-4o Technology</p>
        <p>This report contains detailed analysis of essay submissions using authentic IB assessment criteria.</p>
        <p>Professional grading results for academic excellence and improvement guidance.</p>
        <p>© 2025 CorestoneGrader - All rights reserved</p>
      </div>
    </body>
    </html>
  `;
